...
sed -i "s/v1.30.5/v1.30.0/"  "${dest_config_path}"/vars-conf-cm.yml

sed -i "s/e2e-cluster1-install/${CLUSTER_OPERATION_NAME1}/"  "${dest_config_path}"/kubeanClusterOps.yml


## prepare cluster upgrade job yml --> upgrade from v1.30.0 to v1.30.2
...
sed -i "s/v1.30.5/v1.30.2/"  "${dest_config_path}"/vars-conf-cm.yml

## prepare cluster upgrade job yml --> upgrade from v1.30.2 to v1.30.5
...
sed -i "s/e2e-cluster1-install/${CLUSTER_OPERATION_NAME3}/"  "${dest_config_path}"/kubeanClusterOps.yml
sed -i "s/cluster.yml/upgrade-cluster.yml/" "${dest_config_path}"/kubeanClusterOps.yml
sed -i "s/v1.30.5/v1.30.5/"  "${dest_config_path}"/vars-conf-cm.yml
