...
## prepare cluster upgrade job yml --> upgrade from v1.27.5 to v1.28.0
...
sed -i "s/v1.27.5/v1.28.0/"  "${dest_config_path}"/vars-conf-cm.yml

## prepare cluster upgrade job yml --> upgrade from v1.27.5 to v1.27.6
...
sed -i "s/e2e-cluster1-install/${CLUSTER_OPERATION_NAME3}/"  "${dest_config_path}"/kubeanClusterOps.yml
sed -i "s/cluster.yml/upgrade-cluster.yml/" "${dest_config_path}"/kubeanClusterOps.yml
sed -i "s/v1.27.5/v1.27.6/"  "${dest_config_path}"/vars-conf-cm.yml
